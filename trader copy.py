...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if 'COCONUT' in state.order_depths:
            ...
            sigma_implied = sigma#newton_method_standard(sigma, 5, S=S, K=K, r=r, T=T, C=mid_price_coupon[-1])
            print(time.time()-t)
            t=time.time()
            ...
            if sigma_smooth == 0:
                sigma_smooth = self.sigma_smooth[-1]
            else:
                self.sigma_smooth.append(sigma_smooth)
            print(time.time()-t)
            t=time.time()
